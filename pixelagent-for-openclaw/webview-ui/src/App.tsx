...
function App() {
  ...
  // Listen for gatewayStatus messages (dispatched by vscodeApi.ts from WebSocket)
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === 'gatewayStatus') {
        setGatewayConnected(e.data.connected as boolean);
      }
    };
      ...
      >
        <div style={{ fontSize: 20 }}>
          {gatewayConnected === false
            ? 'Waiting for OpenClaw gateway at ws://127.0.0.1:18789...'
            : 'Loading...'}
        </div>
        {gatewayConnected === false && (
          <div
  ...
  return (
    ...
    >
      ...
      {/* OpenClaw gateway connection status dot */}
