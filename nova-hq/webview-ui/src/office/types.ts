...
export const CharacterState = {
  ...
  WALK: 'walk',
  TYPE: 'type',
  TALK: 'talk',    // walking to deliver message to another agent
  SEARCH: 'search', // web searching (visually distinct from local read)
  THINK: 'think',   // planning / EnterPlanMode
} as const;
...
export interface Character {
  ...
  folderName?: string;
  /** Accumulated idle time (seconds) for extended idle behavior, resets when active */
  idleTime: number;
  /** Timer controlling when next idle behavior fires */
  idleBehaviorTimer: number;
  /** Vertical bounce offset in sprite pixels (0 = no bounce) */
  bounceY: number;
  /** Direction of bounce animation: 1 = going up, -1 = coming down, 0 = none */
  bounceDir: number;
  /** Text displayed in speech bubble, or null if none */
  speechText: string | null;
  /**
   * Speech timer: negative = in transit (TALK walking phase),
   * positive = counting down (bubble visible), 0 = cleared.
   */
  speechTimer: number;
  /** Target agent ID to walk toward in TALK state, or null */
