...
export function createCharacter(
  ...
  return {
    ...
    matrixEffectSeeds: [],
    idleTime: 0,
    idleBehaviorTimer: randomRange(45, 90),
    bounceY: 0,
    bounceDir: 0,
    speechText: null,
    speechTimer: 0,
...
export function updateCharacter(
  ...
  // Universal speech timer countdown (non-TALK states only; TALK manages its own timer)
  if (ch.state !== CharacterState.TALK && ch.speechTimer > 0) {
    ch.speechTimer -= dt;
    if (ch.speechTimer <= 0) {
      ch.speechTimer = 0;
      ch.speechText = null;
  ...
  switch (ch.state) {
    ...
    case CharacterState.IDLE: {
      ...
      if (ch.bounceDir !== 0) {
        const BOUNCE_SPEED = 8; // px/s
        ch.bounceY += ch.bounceDir * BOUNCE_SPEED * dt;
        if (ch.bounceDir < 0 && ch.bounceY <= 0) {
          ch.bounceY = 0;
          ch.bounceDir = 0;
        } else if (ch.bounceDir > 0 && ch.bounceY >= 2) {
          ch.bounceDir = -1; // start coming back down
        }
      }

      // Accumulate extended idle time
      if (!ch.isActive) {
        ch.idleTime += dt;
    ...
    case CharacterState.THINK: {
      // Same FSM as TYPE, but distinct state for visual rendering
      ch.idleTime = 0;
      ch.bounceY = 0;
      ch.bounceDir = 0;
      if (ch.frameTimer >= TYPE_FRAME_DURATION_SEC) {
    ...
    case CharacterState.TALK: {
      ...
      if (ch.path.length > 0) {
        // Walking phase: move toward target
        if (ch.frameTimer >= WALK_FRAME_DURATION_SEC) {
          ch.frameTimer -= WALK_FRAME_DURATION_SEC;
          ch.frame = (ch.frame + 1) % 4;
        }
      ...
      // Path exhausted — arrived at target
      if (ch.speechTimer < 0) {
        // First frame after arrival: start countdown
      ...
      if (ch.speechTimer > 0) {
        // Talking: count down, animate typing pose
        ch.speechTimer -= dt;
        if (ch.frameTimer >= TYPE_FRAME_DURATION_SEC) {
          ch.frameTimer -= TYPE_FRAME_DURATION_SEC;
          ch.frame = (ch.frame + 1) % 2;
        ...
        // Done talking — clear speech and return to seat
        ch.speechText = null;
        ch.targetAgentId = null;
        if (ch.seatId) {
          const seat = seats.get(ch.seatId);
          if (seat) {
            const path = findPath(
...
export function getCharacterSprite(ch: Character, sprites: CharacterSprites): SpriteData {
  switch (ch.state) {
    ...
    case CharacterState.SEARCH:
      // Searching: use reading sprites (looking at screen)
      return sprites.reading[ch.dir][ch.frame % 2];
    case CharacterState.THINK:
      // Thinking/planning: frozen on first typing frame
    ...
    case CharacterState.TALK:
      ...
      // At destination — typing pose (gesturing while talking)
      return sprites.typing[ch.dir][ch.frame % 2];
    case CharacterState.IDLE:
