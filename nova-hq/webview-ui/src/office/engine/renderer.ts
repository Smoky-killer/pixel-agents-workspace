...
export function renderScene(
  ...
  for (const ch of characters) {
    ...
    const drawY = Math.round(offsetY + (ch.y + sittingOffset - (ch.bounceY || 0)) * zoom - cached.height);
...
/** Render a text speech bubble above a character */
function renderTextBubble(
  ...
  const sittingOff = ch.state === CharacterState.TYPE || ch.state === CharacterState.TALK
    ? BUBBLE_SITTING_OFFSET_PX
    : 0;
  ...
  const fontSize = Math.max(7, Math.round(5 * zoom));
  ctx.save();
  ...
  const bubbleCenterX = offsetX + ch.x * zoom;
  // Position bubble above icon bubble area
  const bubbleBottomY =
    offsetY + (ch.y + sittingOff - BUBBLE_VERTICAL_OFFSET_PX) * zoom - (8 * zoom);
  const bubbleX = bubbleCenterX - bubbleW / 2;
  const bubbleY = bubbleBottomY - bubbleH - tailH;
  ...
  ctx.beginPath();
  ctx.moveTo(bubbleCenterX - 3 * zoom, tailTopY);
  ctx.lineTo(bubbleCenterX + 3 * zoom, tailTopY);
  ctx.lineTo(bubbleCenterX, tailTopY + tailH);
  ctx.closePath();
  ctx.fill();

  // Tail border line (left side)
  ctx.globalAlpha = alpha * 0.75;
  ctx.strokeStyle = '#4a9eff';
  ctx.lineWidth = Math.max(0.5, zoom * 0.5);
  ctx.beginPath();
  ...
  ctx.fillText(displayText, bubbleX + padX, bubbleY + padY);
...
export function renderBubbles(
  ...
  for (const ch of characters) {
    // Icon bubble (permission / waiting)
    if (ch.bubbleType) {
      const sprite =
    ...
    // Text speech bubble
    if (ch.speechText !== null && ch.speechTimer > 0) {
      // Fade in over first 0.3s, fade out over last 0.5s
      const fadeIn = Math.min(1, ch.speechTimer > SPEECH_BUBBLE_DURATION_SEC - 0.3
        ? (SPEECH_BUBBLE_DURATION_SEC - ch.speechTimer) / 0.3
        : 1);
      const fadeOut = ch.speechTimer < 0.5 ? ch.speechTimer / 0.5 : 1;
