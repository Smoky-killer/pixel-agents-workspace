...
export class OfficeState {
  ...
  setAgentActive(id: number, active: boolean): void {
    ...
    if (ch) {
      ...
      if (!active) {
        ...
        ch.path = [];
        ch.moveProgress = 0;
        // Reset special states to TYPE so idle/wander FSM kicks in correctly
        if (
          ch.state === CharacterState.SEARCH ||
  ...
  /** Set agent's speech bubble text (shows immediately for given duration) */
  setAgentSpeech(id: number, text: string, duration = SPEECH_BUBBLE_DURATION_SEC): void {
    const ch = this.characters.get(id);
    if (ch) {
      ch.speechText = text;
      ch.speechTimer = duration;
    }
  }

  /** Set agent's CharacterState directly (for SEARCH/THINK transitions) */
  setAgentState(id: number, state: CharacterState): void {
    const ch = this.characters.get(id);
    if (ch && ch.state !== CharacterState.TALK) {
      ch.state = state;
      ch.frame = 0;
      ch.frameTimer = 0;
    }
  ...
  /** Set agent's role for visual styling */
  setAgentRole(id: number, role: 'commander' | 'dispatcher' | 'worker'): void {
    const ch = this.characters.get(id);
    if (ch) ch.role = role;
  }
   ...
   * Returns true if walk initiated, false if agents not found.
   */
  walkToAgent(fromId: number, toId: number, text: string): boolean {
    const fromCh = this.characters.get(fromId);
    const toCh = this.characters.get(toId);
    if (!fromCh || !toCh) return false;

    // Don't interrupt an existing TALK
    if (fromCh.state === CharacterState.TALK) return false;

    fromCh.speechText = text;
    fromCh.targetAgentId = toId;

    const path = this.withOwnSeatUnblocked(fromCh, () =>
      findPath(
        fromCh.tileCol,
        fromCh.tileRow,
        toCh.tileCol,
    ...
    if (path.length > 0) {
      ...
      fromCh.frame = 0;
      fromCh.frameTimer = 0;
      // Negative = in transit (shows bubble only after arrival)
      fromCh.speechTimer = -SPEECH_BUBBLE_DURATION_SEC;
    } else {
      // No path (already adjacent or unreachable) — show speech in place
      fromCh.speechTimer = SPEECH_BUBBLE_DURATION_SEC;
    }
    return true;
  }
